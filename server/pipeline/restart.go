...
func Restart(ctx context.Context, store store.Store, lastPipeline *model.Pipeline, user *model.User, repo *model.Repo, envs map[string]string) (*model.Pipeline, error) {
	...
	// (repo, name, hash), so an unchanged config reuses the existing rows; the
	// name re-sanitize inside findOrPersistPipelineConfig is idempotent for real
	// config filenames, so it does not defeat that dedup.
